function out = PS_expRE
% Matcont system definition file of the PseudoSpectral Discretization of
% the nonlinear RE with exponential nonlinearity
% x(t) = exp(gamma)*int_abar^amax x(t-a)*exp(-x(t-a)) da
...
% --------------------------------------------------------------------------
function dydt = fun_eval(time,state,gamma,abar,aux,tau,M) 
...
% Options
opt_fsolve = optimoptions('fsolve','Display','off','TolFun',1e-16); 
%opt_ode = odeset('RelTol',1e-6,'AbsTol',1e-6); 

% Parameters and functions
d1=1; % dimension of the RE
d2=0; % dimension of the DDE
tau_max=tau;

UM=state(1:d1*M);
%yM=state((d1*M+1):(d1*M+d2)); VM=state((d1*M+d2+1):end);
FM = @(x) 0.5*exp(gamma)*integral(@(theta)interpoly(theta,tau_max*UnitNodes,[x;UM],BaryWeights).*exp(-interpoly(theta,tau_max*UnitNodes,[x;UM],BaryWeights)),-tau,-abar);
%GM = @(x) [];
...
dydt= [(1/tau_max*dMDM_RE)*[KM;state];
