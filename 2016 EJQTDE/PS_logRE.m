function out = PS_logRE
% Matcont system definition file of the PseudoSpectral Discretization of
% the nonlinear RE with quadratic nonlinearity
% x(t) = gamma*int_abar^amax x(t-a)*(1-x(t-a)) da
...
% --------------------------------------------------------------------------
function dydt = fun_eval(time,state,gamma,abar,epsi,aux,tau,M) 
...
FM = @(x) 0.5*gamma*integral(@(theta)interpoly(theta,tau_max*UnitNodes,[x;UM],BaryWeights).*(1-interpoly(theta,tau_max*UnitNodes,[x;UM],BaryWeights)),-tau,-abar);
%GM = @(x) [];
