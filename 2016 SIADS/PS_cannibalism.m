...
% --------------------------------------------------------------------------
function dydt = fun_eval(time,state,beta,abar,aux,tau,M) 
...
FM = @(x) beta*integral(@(theta)interpoly(theta,tau_max*UnitNodes,[x;UM],BaryWeights).*exp(-interpoly(theta,tau_max*UnitNodes,[x;UM],BaryWeights)),-tau,-abar);
GM = [];
