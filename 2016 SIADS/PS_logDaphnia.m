...
% --------------------------------------------------------------------------
function dydt = fun_eval(time,state,beta,abar,r,K,gamma,aux,tau,M) 
...
FM = @(x) beta*yM*integral(@(theta)interpoly(theta,tau_max*UnitNodes,[x;UM],BaryWeights),-tau,-abar)+(1-aux);
GM = @(x) r*yM*(1-yM/K)-(gamma/beta)*FM(x)+(1-aux);
...
% for linear RE and FM=@(x,U):
% Coeff0=1-FM(1,zeros(M,1));
% KM=Coeff0\FM(0,UM);
